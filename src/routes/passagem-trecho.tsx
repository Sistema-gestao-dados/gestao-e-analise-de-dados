...
import { FileSpreadsheet, Upload, Trash2, Play, Download, AlertCircle, CheckCircle2 } from "lucide-react";
...
export const Route = createFileRoute("/passagem-trecho")({
  head: () => ({
    meta: [
      { title: "Módulo 3 — Passagem Trecho FLITS - QSTI | Gestão e Análise de Dados" },
      {
        name: "description",
        content:
          "Importe vários arquivos Excel (aba DADOS1), informe LINHA e SENTIDO e gere um único TXT pronto para importação no QSTI.",
      },
    ],
  }),
  component: PassagemTrecho,
});

function PassagemTrecho() {
  const [files, setFiles] = useState<File[]>([]);
  const [linha, setLinha] = useState("000414L");
  const [sentido, setSentido] = useState<"I" | "V">("I");
  ...
  const handleProcess = async () => {
    ...
    if (!linha.trim()) {
      const msg = "Informe a LINHA antes de processar.";
      setMessages([{ fileName: "", type: "error", message: msg }]);
      toast.error(msg);
      return;
    }
    setProcessing(true);
    ...
    for (let i = 0; i < files.length; i++) {
      const f = files[i];
      const res = await processFile(f, linha.trim(), sentido);
      if (res.error) {
  ...
  const handleExport = () => {
    ...
    if (splitByDate) {
      ...
      for (const [dataSort, recs] of groups) {
        const txt = buildTxt(recs);
        const bytes = encodeWindows1252(txt);
        const fileName = `exportacao_${dataSort}.txt`;
        saveAs(new Blob([bytes as BlobPart], { type: "text/plain;charset=windows-1252" }), fileName);
      }
      toast.success(`${groups.size} arquivo(s) gerado(s) por data.`);
      return;
    }
    const txt = buildTxt(records);
    const bytes = encodeWindows1252(txt);
    saveAs(new Blob([bytes as BlobPart], { type: "text/plain;charset=windows-1252" }), "exportacao.txt");
  };
  ...
  return (
    <div>
        <ModuleNav active="passagem-trecho" />

        <header className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">
            Módulo 3 — Passagem Trecho FLITS - QSTI
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Importe vários arquivos Excel, processe a aba <code className="rounded bg-muted px-1">DADOS1</code>,
            informe LINHA e SENTIDO manualmente (INPUTCARD) e gere um único TXT pronto para importação.
          </p>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            ...
            <CardContent className="space-y-4">
              ...
              >
                ...
                <p className="text-sm font-medium">Arraste arquivos .xlsx aqui</p>
                    ...
                    {files.map((f) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`truncate ${hasError ? "text-destructive font-medium" : ""}`}>
                            {f.name}
                          </span>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>SENTIDO</Label>
                <Select value={sentido} onValueChange={(v) => setSentido(v as "I" | "V")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex items-start gap-2 rounded-md border border-border p-3">
                ...
                <div className="grid gap-1 leading-none">
                  <Label htmlFor="splitByDate" className="cursor-pointer">
                    Exportar por data
                  </Label>
        ...
        {showSuccess && messages.length === 0 && records.length > 0 && (
          <Alert className="mt-6 border-green-500/50 bg-green-500/10 text-green-700 dark:text-green-400">
            ...
            <AlertDescription>
              {records.length.toLocaleString("pt-BR")} registro(s) processado(s) sem erros. Pronto para gerar o TXT.
            </AlertDescription>
          </Alert>
        )}
